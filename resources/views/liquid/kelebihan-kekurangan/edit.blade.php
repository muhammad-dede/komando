...
@section('content')
    ...
    <div class="row mar-b-1rem mar-65-md">
        <div class="col-md-5">
            <h4 class="page-title">Edit Data {!! $kelebihan !!} dan {!! $kekurangan !!}</h4>
        </div>
	</div>
    <div class="row">
        <div class="col-sm-12 col-xs-12">
            <div class="card-box table-responsive comp-tab-blue">
				<form action="{{ route('master-data.kelebihan-kekurangan.update', $data->id) }}" method="post">
					<div class="row">
						<div class="col-md-12 col-xs-12">
								<input type="hidden" name="index_kelebihan" value="{{ $data->details->count() }}">
								...
								<div class="form-group">
									...
									<input type="text" name="judul_kk" class="form-control"
										placeholder="Tulis Judul" value="{{ $data->title }}" required>
								</div>
								<div class="form-group">
									...
									<div>
										<textarea name="deskripsi_kk" rows="20"
											...
											placeholder="Masukan Deskripsi Program">{{ $data->deskripsi }}</textarea>
									</div>
								</div>
								<div class="form-group">
									...
									<select class="select2 form-control form-control-danger" name="status"
										...
										<option value="{{ \App\Enum\KelebihanKekuranganStatus::AKTIF }}"
											{{ $data->status === \App\Enum\KelebihanKekuranganStatus::AKTIF ? 'selected' : '' }}>
											Aktif
										</option>
										<option value="{{ \App\Enum\KelebihanKekuranganStatus::TIDAK_AKTIF }}"
											{{ $data->status === \App\Enum\KelebihanKekuranganStatus::TIDAK_AKTIF ? 'selected' : '' }}>
											Tidak Aktif
										</option>
									</select>
								</div>
								...
								<div class="card-body">
									...
									<div class="row">
                                        <div class="col-md-6 col-xs-12">
                                            <input type="hidden" name="{{ "id_child_$index" }}" value="{{ $item->id }}">
                                            <div class="card">
                                                <div class="card-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-10 col-xs-10">
                                                            <div class="form-group">
                                                                ...
                                                                <div>
                                                                    <textarea name="{{ "deskripsi_kelebihan_$index" }}" rows="10"
                                                                        class="form-control form-control-danger kelebihan"
                                                                        placeholder="Masukan Deskripsi Kelebihan" required>{{ $item->deskripsi_kelebihan }}</textarea>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="card-body">
                                                    <div class="row">
                                                        <div class="col-md-10 col-xs-10">
                                                            ...
                                                            <div class="form-group">
                                                                <div>
                                                                    <textarea name="{{ "deskripsi_kekurangan_$index" }}" rows="10"
                                                                        class="form-control form-control-danger kekurangan"
                                                                        placeholder="Masukan Deskripsi Kekurangan" required>{{ $item->deskripsi_kekurangan }}</textarea>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-1 col-xs-1">
                                                            <em class="fa fa-trash-o delete-kk fa-2x color-red" data-id="{{ $index }}"></em>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
            ...
            var kkCount = $('.wrapper-card .card .card-body textarea.kekurangan').length;
			indexKelebihan.val(kkCount)
			indexKekurangan.val(kkCount)
			...
			$(tinymce.get('deskripsi_kk').getBody()).html($('textarea[name=deskripsi_kk]').val());
            $("#tambah-kk").click(function () {
                var kk = $('.wrapper-card .card .card-body textarea.kekurangan').length;
                kk = kk + 1;
                ...
                $(".append-kk").append("<div class=\"card card-kk" + kk + "\">\n" +
                    ...
                    "                                                        </div>\n" +
                    "                                            <div class=\"col-md-1 col-xs-1\">\n" +
                    "                                                <i class=\"fa fa-trash-o delete-kk fa-2x color-red additional-delete\" data-id=" + kk + "></i>\n" +
                    "                                            </div>\n" +
                    "                                                    </div>\n" +
                    "                                                </div>\n" +
                    "                                            </div>\n" +
        ...
        $(document).on('click', '.additional-delete', function (e) {
        ...
        $(document).on('click', '.delete-kelebihan', function (e) {
            var value = $(this).data("id")
            $('.card-kelebihan-' + value + '').remove();
        });
        $(document).on('click', '.delete-kekurangan', function (e) {
            var value = $(this).data("id")
            $('.card-kekurangan-' + value + '').remove();
        });
