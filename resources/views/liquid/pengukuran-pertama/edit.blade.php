...
@section('content')
	<form
		action="{{ route('penilaian.update', $dataPenilaian->id) }}"
		method="post">
		{{ csrf_field() }}
		{{ method_field('PUT') }}
		<div class="row">
							...
							<tbody>
								@foreach ($resolusi as $index => $item)
									...
									<tr>
										...
										<td>
											<input class="input-range slider-primary" data-index="{{ $index+1 }}" name="{{ 'resolusi_'.($index+1) }}" data-slider-id='ex1Slider' type="text" data-slider-min="0"
											...
											<input type="hidden" name="{{ 'resolusi_alasan_'.($index+1) }}"
												   ...
												   value="{{ $alasan[1] === '' ?'' :  $alasan[1] }}">
										</td>
{{--										<td align="center">--}}
{{--											<input type="radio" name="{{ 'resolusi_'.($index+1) }}" value="{{ \App\Enum\PengukuranPertamaEnum::ST }}" data-toggle="modal"--}}
{{--												data-target="#alasan" data-index="{{ $index+1 }}"--}}
{{--												{{ $filledRes === \App\Enum\PengukuranPertamaEnum::ST ? 'checked' : ''}}>--}}
{{--											<input type="hidden" name="{{ 'resolusi_alasan_'.($index+1) }}"--}}
{{--												id="{{ 'resolusi_alasan_'.($index+1) }}"--}}
{{--												value="{{ (int)$alasan[0] === $index + 1 ? $alasan[1] : '' }}">--}}
{{--										</td>--}}
									</tr>
								@endforeach
							</tbody>
