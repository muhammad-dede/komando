...
@section('title')
    <h4 class="page-title">Persentase Baca Materi</h4>
@stop
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box table-responsive">
                {!! Form::open(['url'=>'report/persentase-baca-materi', 'method' => 'get']) !!}
                ...
                {{-- <div class="col-md-12 align-right" align="right">
                    <a href="{{url('report/persentase-baca-materi/export?bulan='.$selected_bulan.'&tahun='.$selected_tahun)}}" id="export_persentase" 
                ...
                <div class="row m-t-30">
                    <div class="col-md-12">
                        <table id="datatable" class="table table-striped table-bordered">
                            ...
                            <tbody>
                            @foreach($arr_org_clean as $key=>$un )
                                <tr>
                                    <td>{{$un['nama_unit']}}</td>
                                    <td>95%</td>
                                    <td><a href="{{ url('report/monitoring-baca-materi-pegawai?orgeh='.$key.'&bulan='.$selected_bulan.'&tahun='.$selected_tahun.'&minggu_ke=1') }}" target="blank">
                                    {{number_format($un['count_week'][1] / $un['count_pegawai'] * 100,2,',','.')}}%
                                    </a>
                                    </td>
                                    <td><a href="{{ url('report/monitoring-baca-materi-pegawai?orgeh='.$key.'&bulan='.$selected_bulan.'&tahun='.$selected_tahun.'&minggu_ke=2') }}" target="blank">
                                    {{number_format($un['count_week'][2] / $un['count_pegawai'] * 100,2,',','.')}}%
                                    </a>
                                    </td>
                                    <td><a href="{{ url('report/monitoring-baca-materi-pegawai?orgeh='.$key.'&bulan='.$selected_bulan.'&tahun='.$selected_tahun.'&minggu_ke=3') }}" target="blank">
                                    {{number_format($un['count_week'][3] / $un['count_pegawai'] * 100,2,',','.')}}%
                                    </a>
                                    </td>
                                    <td><a href="{{ url('report/monitoring-baca-materi-pegawai?orgeh='.$key.'&bulan='.$selected_bulan.'&tahun='.$selected_tahun.'&minggu_ke=4') }}" target="blank">
                                    {{number_format($un['count_week'][4] / $un['count_pegawai'] * 100,2,',','.')}}%
                                    </a>
                                    </td>
                                    <td><a href="{{ url('report/monitoring-baca-materi-pegawai?orgeh='.$key.'&bulan='.$selected_bulan.'&tahun='.$selected_tahun.'&minggu_ke=5') }}" target="blank">
                                    {{number_format($un['count_week'][5] / $un['count_pegawai'] * 100,2,',','.')}}%
                                    </a>
                                    </td>
                                </tr>
                            @endforeach
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            //     serverSide: true,
            //     ajax: {
            //         url: "{{ route('data_table.report.monitoring.materi') }}",
            //         dataType: "json",
            //         data: function (d) {
